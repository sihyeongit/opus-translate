...
Windows system audio
  ...
  -> src.vad.SileroVAD
  -> src.segment_merge.ShortAudioSegmentMerger
  -> src.asr.FasterWhisperASR
  -> src.quality.TranslationQualityProcessor
...
- `src/segment_merge.py`: 짧은 VAD 세그먼트를 ASR 전에 잠깐 보류하고 다음 세그먼트와 병합한다.
- `src/asr.py`: `faster-whisper`를 인프로세스로 로드해 영어 음성을 문장 단위 텍스트로 변환한다.
- `src/quality.py`: 번역 전 짧은 발화를 병합하고, 번역 후 반복 문장/문장부호/고유명사를 보정한다.
...
- temperature: `0.0` fallback 재시도 방지
- max_new_tokens: `96` runaway decode 방지
- without_timestamps: `True` 내부 timestamp 토큰 생성을 줄여 실시간성 우선
...
현재 번역 품질 보정 설정은 다음과 같다.

- profile: `balanced`
- merge_short_utterances: `True`
- min_standalone_words: `5`
- max_hold_s: `1.6`
- preserve_terms: API, GPT, OpenAI, Windows, YouTube, Python, GitHub 등 주요 고유명사/기술 용어
- phrase_fixes: `resident in memory`, `falling behind the audio`처럼 NLLB가 자주 어색하게 옮기는 표현 보정

현재 ASR 전 짧은 오디오 세그먼트 병합 설정은 다음과 같다.

- profile: `balanced`
- enabled: `True`
- short_segment_ms: `3500`
- max_hold_ms: `1200`
- max_merged_segment_ms: `8000`
...
ASR loop 입구에는 `ShortAudioSegmentMerger`가 있다. 3.5초 미만의 짧은 VAD 세그먼트는 최대 1.2초 보류하고, 다음 세그먼트와 합쳐도 8초 이하라면 하나의 ASR 입력으로 병합한다. 다음 세그먼트가 오지 않으면 보류된 세그먼트를 단독 flush한다.

ASR loop에는 문장 fragment buffering도 있다. VAD가 8초 단위로 긴 발화를 잘라 headless fragment가 생기는 문제를 줄이기 위해, 마지막 문장이 `.?!`로 끝나지 않으면 다음 chunk와 합친다. 단, pending text가 너무 오래 남으면 강제로 flush한다.

ASR 결과가 MT queue로 들어가기 전에는 `TranslationQualityProcessor`가 짧고 문맥 의존적인 발화(`That's why.`, `Right.` 등)를 잠깐 보류했다가 다음 문장과 병합한다. 번역 후에는 한국어 문장부호를 정리하고, 중복 문장을 제거하며, source에 포함된 주요 고유명사가 한국어 음차로 바뀌면 원어 표기로 되돌린다.
...
## 구현 기록: ASR 전 짧은 세그먼트 병합

목표:

- VAD가 만든 짧은 음성 조각을 Whisper에 바로 넣지 않고 잠깐 보류한다.
- 다음 음성 조각이 곧 도착하면 둘을 합쳐 ASR 호출 1번으로 처리한다.
- 짧은 조각마다 Whisper 고정 비용을 반복해서 내는 문제를 줄인다.
- `for both the ship and the tower.` 같은 문맥 없는 조각을 줄여 번역 품질도 개선한다.

문제 배경:

- `max_segment_ms = 8000`은 긴 발화를 최대 8초까지 허용한다는 뜻이지, 모든 발화를 8초로 만든다는 뜻은 아니다.
- 사람이 2~3초 말하고 쉬면 VAD는 여전히 2~3초짜리 세그먼트를 만든다.
- 측정 결과 `seg=2720ms asr=4227ms`처럼 짧은 조각은 실제 음성보다 ASR 시간이 더 길 수 있다.
- 반대로 7~9초 조각은 RTF가 더 안정적이었다.

초기 설정 제안:

```python
merge_short_audio_segments = True
short_segment_ms = 3500
target_min_segment_ms = 3500
max_hold_ms = 1200
min_flush_segment_ms = 1200
max_merged_segment_ms = 8000
```

사용자 프로필 매핑:

- `fast`: 병합 끔. 자막 반응 속도 우선.
- `balanced`: 3.5초 미만 조각을 최대 1.2초 보류하고, 병합 후에도 3.5초 미만이면 한 번 더 모은다. 기본값 후보.
- `quality`: 4.5초 미만 조각을 최대 2.0초 보류. 자연스러운 번역 우선.

구현 위치:

- `src/main.py`의 `_asr_loop` 입구에서 `_asr_q`에서 꺼낸 `SpeechSegment`를 처리한다.
- `_vad_loop`는 VAD iterator 내부에서 다음 세그먼트가 오기 전까지 막힐 수 있어 timeout flush를 정확히 처리하기 어렵다.
- 별도 helper/class는 `src/segment_merge.py`의 `ShortAudioSegmentMerger`로 분리했다.
- `SpeechSegment.audio`는 `numpy.ndarray`이므로 병합은 `np.concatenate([left.audio, right.audio])`로 처리한다.
- `start_ms`는 첫 조각의 값, `end_ms`는 마지막 조각의 값을 사용한다.

1차 구현 범위:

- 짧은 조각 1개만 pending으로 보류한다.
- 다음 조각이 오면 길이 합이 `max_merged_segment_ms` 이하일 때만 병합한다.
- 병합 결과가 `target_min_segment_ms`보다 짧으면 바로 ASR로 보내지 않고 계속 pending으로 둔다.
- 다음 조각이 오지 않고 `max_hold_ms`가 지나면 pending을 그대로 ASR 처리로 보낸다.
- 단, pending이 `min_flush_segment_ms`보다 짧으면 ASR 폭주를 막기 위해 drop한다.
- 병합/flush 시 로그를 남긴다.

예상 로그:

```text
merged short ASR segments: 2720ms + 3040ms = 5760ms
flushed short ASR segment after hold: 2880ms
dropped tiny ASR segment after hold: 672ms (min=1200ms)
```

주의할 점:

- pending을 너무 오래 잡으면 자막 첫 표시가 늦어진다.
- 영화/회의처럼 짧은 대사가 중요한 콘텐츠에서는 `fast` 모드가 필요하다.
- 병합 후 segment가 너무 길어지면 자막이 한꺼번에 나올 수 있다.
- ASR 이후에도 `TranslationQualityProcessor`가 짧은 텍스트 병합을 수행하므로, 오디오 병합과 텍스트 병합이 과하게 중복되지 않게 봐야 한다.

테스트:

- `tests/test_segment_merge.py`에 단위 테스트를 추가했다.
- 2초 + 2초 조각이 4초 조각으로 합쳐지는지 확인한다.
- 2초 pending 후 timeout이 지나면 단독 flush되는지 확인한다.
- 6초 조각은 바로 통과하는지 확인한다.
- 3초 + 7초처럼 합치면 8초를 넘는 경우 병합하지 않는지 확인한다.
- `fast` profile에서 병합이 비활성화되는지 확인한다.
- 실사용 로그에서 `ASR slower than realtime` 빈도와 backlog가 줄었는지 확인한다.
...
5. 자막 품질 변경은 `ShortAudioSegmentMerger`, ASR fragment merge, `TranslationQualityProcessor`, `_is_noise()`, NLLB stateless 번역 제약을 함께 고려한다.
