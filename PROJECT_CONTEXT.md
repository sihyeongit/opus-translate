...
ASR loop에는 문장 fragment buffering이 있다. VAD가 8초 단위로 긴 발화를 잘라 headless fragment가 생기는 문제를 줄이기 위해, 마지막 문장이 `.?!`로 끝나지 않으면 다음 chunk와 합친다. 단, pending text가 너무 오래 남으면 강제로 flush한다.
...
5. 자막 품질 변경은 ASR fragment merge, `TranslationQualityProcessor`, `_is_noise()`, NLLB stateless 번역 제약을 함께 고려한다.
